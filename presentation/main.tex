    ...
    \begin{columns}[T]
        \begin{column}{0.55\textwidth}
            \begin{block}{The Core Idea}
                \textbf{Differential Privacy (DP)} is a mathematical guarantee that the output of a computation reveals very limited information about any single individual in the dataset.
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
             \begin{block}{The Two-Step Process}
                ...
                \begin{enumerate}
                    \item \textbf{Clip:} Each individual gradient is clipped to a maximum norm $C$. This limits the influence of any single data point on the update.
                    \item \textbf{Add Noise:} Gaussian noise is added to the clipped gradient before it is shared.
                \end{enumerate}
...
% --- Slide 3: Why DP Works ---
\begin{frame}{Why Differential Privacy Works?}
    
    \begin{columns}[T]
    
        \begin{column}{0.48\textwidth}
        \begin{block}{Without DP}
        \begin{itemize}
            \item Model updates closely reflect the training data
            \item Shared gradients contain exploitable signals
            \item An attacker can analyze updates to infer sensitive information
        \end{itemize}
        \end{block}
        \end{column}
        ...
        \begin{block}{With DP}
        \begin{itemize}
            \item Gradient clipping limits individual influence
            \item Noise obscures exact data contributions
            \item Shared updates reveal population-level patterns rather than individual records
        \end{itemize}
        \end{block}
        \end{column}
    ...
    \begin{alertblock}{}
        DP does not hide useful learning signals entirely—it ensures that no single individual's data has a significant impact on the shared model update.
    \end{alertblock}
